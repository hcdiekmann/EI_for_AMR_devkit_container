...
static int
battery_bridge_get_property1(struct power_supply *psy,
        enum power_supply_property psp,
        union power_supply_propval *val);

static int
battery_bridge_get_property2(struct power_supply *psy,
        enum power_supply_property psp,
        union power_supply_propval *val);

static int
battery_bridge_ac_get_property(struct power_supply *psy,
        enum power_supply_property psp,
        union power_supply_propval *val);
...
static char *battery_bridge_ac_supplies[] = {
    "BAT0",
    "BAT1",
};

static enum power_supply_property battery_bridge_properties[] = {
...
static enum power_supply_property battery_bridge_ac_properties[] = {
...
static struct power_supply_desc descriptions[] = {
    {
        ...
        .properties = battery_bridge_properties,
        .num_properties = ARRAY_SIZE(battery_bridge_properties),
        .get_property = battery_bridge_get_property1,
    },

    {
        ...
        .properties = battery_bridge_properties,
        .num_properties = ARRAY_SIZE(battery_bridge_properties),
        .get_property = battery_bridge_get_property2,
    },

    {
        .name = "AC0",
        .type = POWER_SUPPLY_TYPE_MAINS,
        .properties = battery_bridge_ac_properties,
        .num_properties = ARRAY_SIZE(battery_bridge_ac_properties),
        .get_property = battery_bridge_ac_get_property,
    },
};
...
static struct power_supply_config configs[] = {
    ...
    { },
    {
        .supplied_to = battery_bridge_ac_supplies,
        .num_supplicants = ARRAY_SIZE(battery_bridge_ac_supplies),
    },
};
...
{
    static char *message = "battery_bridge battery information!";
...
control_device_write(struct file *file, const char *buffer, size_t count, loff_t *ppos)
{
    ...
    if(*ppos != 0) {
        printk(KERN_ERR "writes to /dev/battery_bridge must be completed in a single system call\n");
        return -EINVAL;
    }

    if(count > 1024) {
        printk(KERN_ERR "Too much data provided to /dev/battery_bridge (limit 1024 bytes)\n");
        return -EINVAL;
    }
    ...
    while((newline = memchr(buffer_cursor, '\n', bytes_left))) {
        *newline = '\0';
        ...
        status = handle_control_line(buffer_cursor, &ac_status, battery_bridge_statuses);
    ...
    handle_charge_changes(ac_status, &battery_bridge_statuses[0]);
    handle_charge_changes(ac_status, &battery_bridge_statuses[1]);
...
static int
battery_bridge_generic_get_property(struct power_supply *psy,
        enum power_supply_property psp,
        union power_supply_propval *val,
        ...
        union power_supply_propval *val)
{
    switch (psp) {
        case POWER_SUPPLY_PROP_MODEL_NAME:
            val->strval = "battery_bridge battery 1";
            break;
        case POWER_SUPPLY_PROP_SERIAL_NUMBER:
            val->strval = "12345678";
            break;
        default:
            return battery_bridge_generic_get_property(psy, psp, val, &battery_bridge_statuses[0]);
    }
    return 0;
}
        ...
        union power_supply_propval *val)
{
    switch (psp) {
        case POWER_SUPPLY_PROP_MODEL_NAME:
            val->strval = "battery_bridge battery 2";
            break;
        case POWER_SUPPLY_PROP_SERIAL_NUMBER:
            val->strval = "12345678";
            break;
        default:
            return battery_bridge_generic_get_property(psy, psp, val, &battery_bridge_statuses[1]);
    }
    return 0;
}
...
battery_bridge_init(void)
{
    ...
    for(i = 0; i < ARRAY_SIZE(descriptions); i++) {
        ...
        if(IS_ERR(supplies[i])) {
            printk(KERN_ERR "Unable to register power supply %d in battery_bridge\n", i);
            goto error;
        }
    }

    printk(KERN_INFO "loaded battery_bridge module\n");
    return 0;
...
battery_bridge_exit(void)
{
    ...
    printk(KERN_INFO "unloaded battery_bridge module\n");
}

module_init(battery_bridge_init);
module_exit(battery_bridge_exit);
